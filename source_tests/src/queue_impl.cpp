...
void LIST_INSERT_AFTER_impl(IntegerLISTEntry *listelm, IntegerLISTEntry *elm) {
    do { ;
        if (((((elm))->links.le_next) = (((listelm))->links.le_next)) != NULL)
            (((listelm))->links.le_next)->links.le_prev = &(((elm))->links.le_next);
        (((listelm))->links.le_next) = (elm);
        (elm)->links.le_prev = &(((listelm))->links.le_next);
    }
    while (0);
}
...
void LIST_INSERT_BEFORE_impl(IntegerLISTEntry *listelm, IntegerLISTEntry *elm) {
    do { ;
        (elm)->links.le_prev = (listelm)->links.le_prev;
        (((elm))->links.le_next) = (listelm);
        *(listelm)->links.le_prev = (elm);
        (listelm)->links.le_prev = &(((elm))->links.le_next);
    }
    while (0);
}
...
void LIST_INSERT_HEAD_impl(myLISTHead *head, IntegerLISTEntry *elm) {
    do { ;
        if (((((elm))->links.le_next) = (((head))->lh_first)) != NULL)
            (((head))->lh_first)->links.le_prev = &(((elm))->links.le_next);
        (((head))->lh_first) = (elm);
        (elm)->links.le_prev = &(((head))->lh_first);
    }
    while (0);
}
...
IntegerLISTEntry *LIST_PREV_impl(IntegerLISTEntry *elm, myLISTHead *head) {
    return ((elm)->links.le_prev == &(((head))->lh_first) ? NULL : ({
        const volatile __typeof(((IntegerLISTEntry *) 0)->links.le_next) *__x = ((elm)->links.le_prev);
...
void LIST_REMOVE_impl(IntegerLISTEntry *elm) {
    do { ;;;;
        if ((((elm))->links.le_next) != NULL) (((elm))->links.le_next)->links.le_prev = (elm)->links.le_prev;
        *(elm)->links.le_prev = (((elm))->links.le_next);;;
    }
    while (0);
}
...
void LIST_SWAP_impl(myLISTHead *head1, myLISTHead *head2) {
    do {
        ...
        if ((swap_tmp = (((head1))->lh_first)) != NULL) swap_tmp->links.le_prev = &(((head1))->lh_first);
        if ((swap_tmp = (((head2))->lh_first)) != NULL) swap_tmp->links.le_prev = &(((head2))->lh_first);
    }
    while (0);
}
