...
struct first: rc::state::Command<LIST_model, myLISTHead> {
...
struct insert_head : rc::state::Command<LIST_model, myLISTHead> {
...
struct insert_after : rc::state::Command<LIST_model, myLISTHead> {
...
struct remove_element : rc::state::Command<LIST_model, myLISTHead> {
    unsigned int index = *rc::gen::arbitrary<unsigned int>();
...
struct swap : rc::state::Command<LIST_model, myLISTHead> {
...
struct concatenate : rc::state::Command<LIST_model, myLISTHead> {
...
struct empty : rc::state::Command<LIST_model, myLISTHead> {
    void run(const LIST_model &model, myLISTHead &head1) const override {
...
struct foreach : rc::state::Command<LIST_model, myLISTHead> {
    void run(const LIST_model &model, myLISTHead &head) const override {
...
struct foreach_from : rc::state::Command<LIST_model, myLISTHead> {
...
TEST(LIST_model, sequenceOfOperationsAlwaysResultsInExpectedState){
    RC_ASSERT(rc::check([] {
        LIST_model model;
        myLISTHead head{};
        LIST_INIT_impl(&head);
